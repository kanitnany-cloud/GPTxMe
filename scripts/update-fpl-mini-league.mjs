...
async function main() {
  ...
  const json = {
    ...
    teams: teams.map((team) => ({
      ...
      weekly_history: (team.historyCurrent || []).map((gw) => ({
        event: gw.event,
        points: gw.points,
        ...
        bank: (gw.bank ?? 0) / 10,
        team_value: (gw.value ?? 0) / 10,
        event_transfers: gw.event_transfers ?? 0,
        event_transfers_cost: gw.event_transfers_cost ?? 0,
        points_on_bench: gw.points_on_bench ?? 0,
      })),
      chip_history: (team.chips || []).map((chip) => ({
        name: chip.name,
        event: chip.event,
        time: chip.time,
      })),
      transfer_history: (team.allTransfers || []).map((transfer) => ({
